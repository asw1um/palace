...
import { LogIn, LogOut, Plus, Search, Users } from '@/lib/icons';
      ...
      {loading ? (
        ...
        <div className="grid grid--cards" ref={gridRef}>
          {rows.map((club) => {
            ...
            return (
              <article key={club.id} className="card" onClick={() => router.push(`/clubs/${club.id}`)}>
                <div className="row gap-3" style={{ marginBottom: 'var(--space-3)' }}>
                  <Avatar src={club.image_url} name={club.name} size={40} />
                  <div className="grow" style={{ minWidth: 0 }}>
                    <h3 className="truncate" style={{ fontSize: 'var(--text-md)' }}>
                      {club.name}
                    </h3>
                ...
                <p className="muted clamp-2" style={{ fontSize: 'var(--text-sm)', minHeight: 40 }}>
