          ...
          {ordered.map((l) => {
            ...
            return (
              <article
                ...
                onDrop={() => onDrop(l.id)}
                onClick={() => !editing && renaming !== l.id && router.push(`/lists/${l.id}`)}
              ...
              >
                <div className="row gap-2" style={{ marginBottom: 'var(--space-3)' }}>
                  ...
                  {renaming === l.id ? (
                    ...
                    <h3 className="grow truncate" style={{ fontSize: 'var(--text-md)' }}>
                ...
                <div className="row gap-1" style={{ marginBottom: 'var(--space-3)' }}>
                  {posters.length === 0 && (
                    <div
                      className="panel panel--inset row center"
                      style={{ height: 72, width: '100%', fontSize: 'var(--text-xs)', color: 'var(--text-faint)' }}
                    >
                      Empty list
                    </div>
                  )}
                  {posters.map((p) => (
                    <div
                      key={p.id}
                      style={{
                        width: 48, height: 72, borderRadius: 'var(--r-sm)', overflow: 'hidden',
                        background: 'var(--bg-subtle)', flexShrink: 0,
                      }}
                    >
                      {p.poster_url && <Image src={p.poster_url} alt="" width={48} height={72} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                    </div>
                  ))}
