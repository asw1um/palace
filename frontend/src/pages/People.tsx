...
export default function People() {
  ...
  useEffect(load, []);
  useEffect(() => {
    if (!loading) staggerIn(document.querySelectorAll('.person-card'), { y: 14, each: 30 });
  }, [loading, tab, query]);
      ...
      {loading ? (
        ...
        <div className="grid grid--cards">
          {rows.map((u) => {
            ...
            return (
              <article key={u.id} className="card person-card" onClick={() => navigate(`/profile/${u.username}`)}>
                ...
                <p className="muted clamp-2" style={{ fontSize: 'var(--fs-13)', marginTop: 'var(--sp-3)', minHeight: 36 }}>
                  {u.bio || 'No bio yet.'}
                </p>

                <div className="row gap-2" style={{ marginTop: 'var(--sp-3) ' }} onClick={(e) => e.stopPropagation()}>
                  {rel?.status === 'incoming' ? (
                    <>
                      <Button
                        size="sm"
