...
export default function Profile() {
  ...
  useEffect(() => {
    if (!loading) staggerIn(document.querySelectorAll('.stat'), { y: 12, each: 55 });
  }, [loading, stats]);
  ...
  return (
    <>
      <section className="panel" style={{ overflow: 'hidden' }}>
        <div
          style={{
            ...
            background: profile.banner
              ...
              : 'linear-gradient(120deg, color-mix(in oklab, var(--accent) 55%, transparent), color-mix(in oklab, var(--accent-2) 55%, transparent))',
          }}
        >
          {isMe && (
            <Button
              ...
              variant="ghost"
              icon={<Camera size={14} />}
              style={{ position: 'absolute', right: 12, top: 12, background: 'var(--scrim)' }}
        ...
        <div className="row gap-4 wrap" style={{ padding: 'var(--sp-5)', marginTop: -46 }}>
          ...
          <div className="grow" style={{ minWidth: 220, marginTop: 'var(--sp-6)' }}>
            <h1>{profile.nickname ?? profile.username}</h1>
            <div className="faint" style={{ fontSize: 'var(--fs-13)' }}>
              @{profile.username}
              {profile.created_at ? ` · joined ${timeAgo(profile.created_at)}` : ''}
            </div>
            <p className="muted" style={{ marginTop: 'var(--sp-2)', maxWidth: '68ch' }}>
      ...
      {tab === 'overview' && (
        <div className="stack gap-6">
          ...
          {isMe && pinned.length > 0 && (
            // Pinned lists sit above reviews so the good stuff is first. — issue #24
    ...
    <div className="stack gap-2">
      {review.title && (
        ...
        >
          <div
            style={{
              width: 28, height: 42, borderRadius: 'var(--r-xs)', overflow: 'hidden',
              background: 'var(--surface-3)', flexShrink: 0,
            }}
          >
            {review.poster_url && (
              <img src={review.poster_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            )}
          </div>
          <span style={{ fontWeight: 650, fontSize: 'var(--fs-13)' }}>{review.title}</span>
